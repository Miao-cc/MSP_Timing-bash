...
if [ ${#Cal1Name} -gt 0 ] || [ ${#calzap} -gt 0 ];then
    if [ ${#Cal1Name} -gt 0 ];then
        ...
        dspsr -A -t ${cpuNum} -c ${CalPeriod} -L 2.01326592 -O ${CalFilePrefix} -e rf ${Cal1Name}
        echo "Write to file ${CalFilePrefix}.rf"
        pam -t 5 -m ${CalFilePrefix}.rf
        # Zap channels
        echo "Zapping"
        zap.psh -e zap ${CalFilePrefix}.rf
        echo "===== CAL FOLD FINISH====="
    elif [ ${#calzap} -gt 0 ];then
        echo "===== PAM CAL ZAP FILE ====="
        zapFile=${calzap}
        CalFilePrefix=${calzap%.*}
#        # sum time by factor 5 
#        pam -t 5 -e ucf ${zapFile}
#        echo "Write to file ${CalFilePrefix}.ucf"
        # correct the header
        psredit -c type='PolnCal',rcvr:name=19BEAM -e ucf ${calzap}
...
if [ ${#PulsarName} -gt 0 ] || [ ${#psrzap} -gt 0 ];then # fold file with given par file or period and DM
    ...
    pac -d ${Calfile} -c -Z -n q -U ${PsrFilePrefix}.feed
    echo "Write to file ${PsrFilePrefix}.calibP"
    # sum freq to 32 channels
    pam -pf 1024 -e pf4 ${PsrFilePrefix}.calibP
