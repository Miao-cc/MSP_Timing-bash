...
#time=np.linspace(0,obstime,(len(cutAA.mean(0))))
time=np.linspace(0,NCHAN,(len(cutAA.mean(0))))
plt.subplot(211)
print cutAA.shape
#plt.plot(time,cutAA.mean(1),'k')
plt.plot(time,cutAA.mean(0),'k')
plt.title('POLA Bandpass (output power spectrum)',font1)
#plt.ylabel('Intensity [Arbitrary Units]',font2)
plt.xticks([])
plt.tick_params(labelsize=18)
plt.subplot(212)
#plt.plot(time,cutBB.mean(1),'k')
plt.plot(time,cutBB.mean(0),'k')
plt.title('POLB Bandpass (output power spectrum)',font1)
plt.xlabel('duration Channel(1-4096)',font1)
plt.ylabel('Intensity [Arbitrary Units]',font2)
plt.subplots_adjust(wspace=0.0, hspace=0.25)
plt.tick_params(labelsize=18)
figname = datapath[:-5] + 'checkChan.png'
