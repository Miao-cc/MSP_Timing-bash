    ...
    mjds, mjd_floats, res, errors = zip(
        *[
            (
                r[0],                    # mjd
                r[0].mjd,                # mjd float
                r[1].to_value(u.us),     # residual 
                r[2].to_value(u.us),     # error
            )
            for r in res
        ]
    )
...
def read_res_file(filename):
    ...
    for line in f.readlines():
        ...
        arg1, arg2 = MJD
        mjd = time.Time(arg1, arg2, scale='utc', format="mjd", precision=9)
        res.append([mjd, residual, error])
    return res
...
class Res:
    ...
    def __init__(self, resfile):
        ...
        reslist = read_res_file(resfile)

        if reslist is None:
            raise ValueError("No Res found!")

        self.table = build_table(reslist, filename=self.filename)
